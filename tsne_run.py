...
testlabels = np.load('./npys/e2/testlabels.npy').astype(np.float64)[:limit]
advoutput = np.load('./npys/e2/advoutput.npy').astype(np.float64)[:limit]
advdata = np.load('./npys/e2/advdata.npy').astype(np.float64)[:limit]
print(advdata.shape)
print(testlabels.shape)
print(advoutput.shape)
...
X_2d = []
data=traindata[:10000]
if exists("./npys/embedding.npy"):
    X_2d = np.load('./npys/embedding.npy').astype(np.float64)
else:
    tsne = TSNE(n_components=2, random_state=3,perplexity=100,verbose=True)
